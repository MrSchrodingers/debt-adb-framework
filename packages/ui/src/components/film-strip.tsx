      ...
      <div className="space-y-3">
        ...
        {frames.length === 1 && (
          <div className="flex items-start gap-2 rounded-lg border border-zinc-800/60 bg-zinc-900/40 px-3 py-2">
            ...
            <span className="text-xs text-zinc-500">{t('filmStrip.limitationNote')}</span>
          </div>
        )}
          ...
          {availableFrames.map((frame, idx) => (
            ...
            >
              <img
                ...
                className="h-16 w-10 object-cover bg-zinc-800"
        ...
        <div className="relative rounded-xl border border-zinc-700/40 overflow-hidden bg-zinc-950/60">
          <img
            src={current.url!}
            alt={`${t('filmStrip.frame')} ${activeIdx + 1} ${t('filmStrip.of')} ${frameCount} — ${messageId.slice(0, 8)}`}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-zinc-800 border border-zinc-700/40 px-3 py-2 text-xs text-zinc-300 hover:bg-zinc-700 disabled:opacity-40 disabled:cursor-not-allowed transition"
          >
            {playing ? (
