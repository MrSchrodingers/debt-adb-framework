  ...
  describe('mapAccounts', () => {
    ...
    it('detects both WA and WABA on same profile', async () => {
      fakeShell.mockImplementation(async (_serial: string, cmd: string) => {
        ...
        if (cmd.includes('com.whatsapp_preferences_light') && cmd.includes('user 0'))
          return '<map><string name="registration_jid">5543968350100@s.whatsapp.net</string></map>'
        if (cmd.includes('com.whatsapp.w4b') && cmd.includes('user 0'))
          return '<map><string name="registration_jid">5543968350101@s.whatsapp.net</string></map>'
        return ''
      })
    ...
    it('iterates user profiles 0, 10, 11, 12', async () => {
      ...
      fakeShell.mockImplementation(async (_serial: string, cmd: string) => {
        ...
        if (cmd.includes('pm list packages') && cmd.includes('--user 10'))
          return 'package:com.whatsapp'
        if (cmd.includes('shared_prefs') && cmd.includes('user 10'))
          return '<map><string name="registration_jid">5543968350095@s.whatsapp.net</string></map>'
        return ''
      })
